...
from opening_trainer.tree_session import merge_side_trees, variation_outline
...
def _merged(*trees):
    return merge_side_trees(list(trees), chess.BLACK)


def test_two_named_groups_in_tree_order():
    out = variation_outline(_merged(_black("Klassisch", KLASS),
...
def test_gap_leaf_is_flagged_and_counted():
    ...
    out = variation_outline(_merged(_black("Lücke", gap_line)), chess.BLACK)
    g = out[0]
    assert g["gaps"] == 1
...
def test_group_with_two_lines_branches_and_marks_preview_cut():
    ...
    out = variation_outline(_merged(a, b), chess.BLACK)
    assert len(out) == 1
    g = out[0]
    assert g["lines"] == 2
...
def test_user_move_flag_matches_side():
    out = variation_outline(_merged(_black("Klassisch", KLASS)), chess.BLACK)
